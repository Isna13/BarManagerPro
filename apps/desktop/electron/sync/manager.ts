...
export class SyncManager {
  ...
  private criticalSyncInterval: NodeJS.Timeout | null = null;
  // 🔴 CORREÇÃO F5: Contador para sync de settings (menos frequente)
  private _settingsSyncCounter: number = 0;
  ...
  async login(credentials: { email: string; password: string }) {
    ...
    try {
      ...
      try {
        ...
        const localUser = this.dbManager.getUserByEmail(credentials.email) as any;
   ...
   */
  async checkServerCashBox(branchId: string): Promise<{
    hasOpenBox: boolean;
    serverBox: any | null;
  }> {
  ...
  async openCashBoxWithServerCheck(data: {
    ...
    openingCash: number;
    notes?: string;
  }): Promise<{ success: boolean; cashBox?: any; error?: string }> {
   ...
   */
  async getCurrentCashBoxWithServerCheck(branchId?: string): Promise<any> {
   ...
   */
  private async syncCriticalEntities() {
    if (!this._isOnline || !this.token || this.token === 'offline-token') {
   ...
   */
  private async pullCriticalUsers() {
    try {
   ...
   */
  private async bulkSyncSales(salesItems: SyncItem[]): Promise<{ success: number; failed: number }> {
    ...
    try {
      ...
      for (let i = 0; i < salesItems.length; i++) {
        ...
        const hasError = result.errors?.some((e: any) => e.item?.entityId === item.entity_id);
        ...
        if (!hasError) {
          ...
          const error = result.errors.find((e: any) => e.item?.entityId === item.entity_id);
   ...
   */
  async fetchFromServer(endpoint: string): Promise<{ success: boolean; data?: any; error?: string }> {
    if (!this.isOnline) {
          ...
          try {
            ...
            if (existing) {
              ...
              if (saleId) {
                ...
                `).get(saleId) as { id: string; debt_number: string } | undefined;
